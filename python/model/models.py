...
#                 attn_module = attn.Attention2D if FLAGS.attn_depth>1 else attn.Attention # attn_module = attn.HighwayAttention
#                 attn_word = attn_module(FLAGS, seq_len=word_level_lengths)
#                 word_level_output = attn_word(word_encoder_output); self.log_tensor(word_level_output, name='word_level_output')
#                 if self.FLAGS.attn_vis: self._alpha_word = attn_word.alpha
#                 if FLAGS.attn_depth>1: self._penalty += attn_word.penalty
#                 self.update_tensors(attn_word)
    #################################################################################

#             if FLAGS.attn_size>0:
#                 ''' attention pooling (sum over sentences) '''
#                 with tf.variable_scope('attention') as scope:
#                     attn_sent = attn.Attention(FLAGS, seq_len=self.sentence_lengths) #attn_sent = attn.Attention2D(FLAGS) # attn_sent = attn.HighwayAttention(FLAGS)    
#                     sentence_level_output = attn_sent(sentence_encoder_output)
#                     self.update_tensors(attn_sent)
#                     
#                     if self.FLAGS.attn_vis: self._alpha_sent = attn_sent.alpha
#                     #self._penalty = attn_sent.penalty
#             ## mean pool
#             elif FLAGS.attn_size==0:
#                 sentence_level_output = U.dynamic_mean(sentence_encoder_output, self.sentence_lengths, pad=FLAGS.pad)
#             ## final rnn state?
#             elif FLAGS.attn_size==-1:
#                 sentence_level_output = rnn_module_sent.final_rnn_state

    ###########################
    #dim = tf.shape(word_level_output)[-1]
    #dim = word_level_output.get_shape().as_list() [-1]
    #dim = FLAGS.rnn_size * (2 if FLAGS.bidirectional else 1)
    #dim = FLAGS.attn_depth * FLAGS.rnn_size * (2 if FLAGS.bidirectional else 1)
    #dim = FLAGS.attn_depth * FLAGS.rnn_size * 5
    #         else:
    #             dim = word_level_output.get_shape().as_list() [-1]
    
    #### TEST highway layer
    #hw = mc.Highway(output_size=word_encoder_output.shape[-1].value)
    #word_encoder_output = hw(word_encoder_output)
